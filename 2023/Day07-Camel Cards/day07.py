...
# Returns dictionary
#	- key:    card face/number
#	- value:  number of times that card appears in the hand
...
def handle_wild_card(cards, card_counts, deck,debug_hand):
	...
	wild_card_i = cards.find(wild_card)
	...
	'''
	# TODO: change how Wild cards are handled by the cards before and after
	#	- to handle "Two Pair": ACCOUNT for cards AFTER wild_card - just grab "highest value"
	# What it already does:
	# 	- Get cards with highest count 
	#	- sort suits by whichever comes first
	# 	- change wild_card to that suite
	top_suits = wild_card
	# Only need to loop twice b/c if first loop doesn't work we just need to try one more time. 
	counts_grouped = list(set([ x[1] for x in card_counts ]))
	counts_grouped.sort(reverse=True)
	current_count_group = counts_grouped.pop(0)
	# Card and first instance it appears in the hand. 
	cards_in_group = [ [x[0], cards.find(x[0])] for x in card_counts if x[1] == current_count_group ]
	# Sort the cards in this group by when they first appear in the 
	cards_in_group.sort(key=lambda x: x[1] )
	for i in range(2):
		print("eh?")
	...
	# Get the cards with the highest count (that aren't the wild card)
	top_suits_by_count = [ x[0] for x in card_counts if x[1] == card_counts[0][1] and x[0] != wild_card ]
	# Find the indexes of all the "highest cards"
	i = [ [ x[0], cards.find[x[0]] ] for x in card_counts if x[1] == card_counts[0][1] ]
	...
	# Five of a kind, just replace with the highest value card possible
	if len(card_counts) == 1:
		replacement = deck.unique_cards[-1]
	else:
...
def hand_scoring(cards, deck, debug_hand=False):
	...
	hand_type_score = len(hand_scores) - hand_scores.index(True) -1
	hand_score_rebase = hand_type_score * deck.base ** len(cards)
	if debug_hand: 
		# logging.debug('\t' + f'{hand_score_str=} :: {hand_type_score}*{deck.base ** len(cards):,}={hand_score_rebase:,}')
		logging.debug('\t' + f'{hand_score_str=} :: {hand_type_score}')
		logging.debug('\t' + f'{sub_score=}')

	logging.getLogger().setLevel(previous_logging_level)
	return hand_score_str, hand_type_score, sub_score 
...
def betting(problem_num, hands_to_debug=0, debug_criteria = lambda hand:True):
	...
	if problem_num == 1:
		...
		unique_cards = ['J','2','3','4','5','6','7','8','9','T','Q','K','A']
	...
	for player in players:
		...
		hand_score_str, hand_type, score_sub = hand_scoring(hand,deck,debug_hand)
		hand_base= [hand_score_str, hand, hand_type, score_sub ]

		hand_changed=[hand_score_str,'no change',hand_type, score_sub ]
		if problem_num==2: 
			...
			hand_score_str, hand_type, score_sub  = hand_scoring(new_cards,deck,debug_hand)
			hand_changed = [hand_score_str,new_cards,hand_type, score_sub]
	...
	if problem_num == 1:
		players.sort(key=lambda x:(x[2][2], x[2][3]))
	elif problem_num == 2:
		players.sort(key=lambda x:(x[2][2], x[1][3]))
	# logging.debug(pprint.pformat(players[:hands_to_debug]))
	...
	h_count = 0
	for player in players: 
		debug_hand = debug_criteria(player[1][1]) # Hand before 
		if not debug_hand: continue
		h_count += 1
		if h_count>hands_to_debug: break
...
def handler():
	problems = {
		...
		'2' : lambda: betting(problem_num=2, hands_to_debug=10, debug_criteria=lambda hand: 'J' in hand) # Answer is: 249,666,369
	}
